...
class AudioManager:
    ...
    def play_audio(self, file_path, sleep_during_playback=False, delete_file=False, play_using_music=False):
        """
        Parameters:
    ...
    def record_audio(self, audio_device=None, end_recording_key='=', time_limit=None):
        # Records audio from an audio input device.
        # Example device names are "Line In (3- High Definition Audio Device)", "Sample (TC-Helicon GoXLR)", or just leave empty to use default mic
        ...
        # end_recording_key is the key that stops the recording.
        # time_limit is the number of seconds until the recording automatically ends. If one isn't provided, it will record until the end key is pressed.
        ...
        # Wait until end key is pressed, or until time limit is up
        start_time = time.time()
        while True:
            # Check if the key is pressed
            if keyboard.is_pressed(end_recording_key):
                print("[red]Finished recording (end_key pressed).")
                break
            # Check if the time limit is up
            if time_limit is not None:
                elapsed_time = time.time() - start_time
                if elapsed_time >= time_limit:
                    print("[red]Finished recording (time_limit reached).")
                    break
...
if __name__ == '__main__':
    ...
    TEST_MP3 = "TestAudio_MP3.mp3"
    TEST_WAV = "TestAudio_WAV.wav"
    ELEVENLABS_MP3 = "ElevenlabsMP3_Test.mp3"
    ELEVENLABS_WAV = "ElevenlabsWAV_Test.wav"
    POLLY_MP3 = "PollyMP3_Test.mp3"
    AZURE_WAV = "AzureWAV_Test.wav"
    
    # MP3 Test
    # audio_manager.play_audio(TEST_MP3)
    ...
    # x = 10
    # while x > 0:
    #     audio_manager.play_audio(TEST_MP3,False,False,False)
    #     time.sleep(0.1)
    #     x -= 1
    # print("Sleeping until next file")
    # time.sleep(3)
    ...
    # Wav file tests
    # audio_manager.play_audio(TEST_WAV)
    # print("Sleeping until next file")
    # time.sleep(3)
    ...
    # x = 10
    # while x > 0:
    #     audio_manager.play_audio(TEST_WAV,False,False,False)
    #     time.sleep(0.1)
    #     x -= 1
    # print("Sleeping until next file")
    # time.sleep(3)
    ...
    #     await audio_manager.play_audio_async(TEST_MP3)
    #     time.sleep(1)
    #     await audio_manager.play_audio_async(TEST_WAV)
    #     time.sleep(1)
    ...
    # Combining files tests
    # combined_file = audio_manager.combine_audio_files([TEST_MP3, TEST_WAV, ELEVENLABS_MP3, ELEVENLABS_WAV, POLLY_MP3, AZURE_WAV])
    # audio_manager.play_audio(combined_file, True, False, True)
    # combined_file = audio_manager.combine_audio_files([TEST_WAV, ELEVENLABS_WAV, AZURE_WAV])
    # audio_manager.play_audio(combined_file, True, False, True)
    # combined_file = audio_manager.combine_audio_files([TEST_MP3, ELEVENLABS_MP3, POLLY_MP3])
    # audio_manager.play_audio(combined_file, True, False, True)

    # Recording audio tests
    print("[green]Started recording on default mic with no time limit")
    audio_manager.record_audio(audio_device="Line In (3- High Definition Audio Device)")
    print("[green]Started recording on default mic with 10 second time limit")
    audio_manager.record_audio(time_limit = 10)
    print("Finished recording!!")
    print("[green]Started recording on Sample GoXLR mix! (i.e. the music channel)")
    device_name = "Sample (TC-Helicon GoXLR)"
    audio_manager.record_audio(audio_device=device_name)
    print("Finished recording!!")
